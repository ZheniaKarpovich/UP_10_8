...
public class MudClient {
    ...
    public static void main(String[] args) {
        try {
            ...
            boolean adminFlag = Boolean.getBoolean(getLine("Are you admin?(true/false) ")); //сделать getline для этого 
            ...
            MudPerson me = new MudPerson(name, description, myout, adminFlag);
        ...
        for(;;) {  // Loop until the user types "quit"
            try {    // Catch any exceptions that occur in the loop
                ...
                // Describe news content
                	...
                	Vector newsTitles = location.getNewsTitles(); 
                	System.out.println("News:");
                	for(int j = 0; j < newsTitles.size(); j++) {
                		System.out.println(i + ". " + newsTitles.get(i));
                	}
                } else if (cmd.equals("examine")) 
                    System.out.println(location.getNewsContent(arg));
                ...
                //комментирвоание новостей и срач с другими пользователями
                // SAY: Say something to everyone 
                else if (cmd.equals("comment")) location.comment(me, arg);
                // DO: Do something that will be described to everyone
                else if (cmd.equals("do")) location.comment1(me, arg);
                ...
                // CREATE: Create a new thing in this place ?? create news
                else if (cmd.equals("create")) {
                    if (arg.length() == 0)
                        throw new IllegalArgumentException("name expected");
                    if(!me.getAdminFlag())
                    	throw new Mud.NoAdminLaw();
                    String desc = getMultiLine("Please describe the " +
					       arg + ": ");
                    location.createNews(me, arg, desc);
                }
                // DESTROY: Destroy a named thing ?? destroy news
                else if (cmd.equals("destroy")) {
                    if(!me.getAdminFlag())
                    	throw new Mud.NoAdminLaw();
                	location.destroyNews(me, arg);
                }
			...
			getLine("What is the place name in that MUD?: ");
                ...
                // if the password is correct
                    ...
                    if(!me.getAdminFlag())
                    	throw new Mud.NoAdminLaw();
                    location.removeOldNews(me);
                }
                // FIND: find news by date,
                else if (cmd.equals("find")) {
                    if (arg.length() != 1) 
                       throw new IllegalArgumentException("date for find expected");
                    SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
                    Date date = formatter.parse(arg);
                    Vector res = location.findNewsByDay(me, date);
                }
                // QUIT: Quit the game
            ...
            // Handle the many possible types of MudException
                ...
                else if (e instanceof Mud.NoAdminLaw) 
                    System.out.println("You don't have admin law."); 
	...
	throws RemoteException, Mud.MudException
    {
        ...
        Vector newsTitles = p.getNewsTitles();               // List of things here
        ...
        System.out.print("News titles here: ");
        for(int i = 0; i < newsTitles.size(); i++) {      // Display list of newsTitles
            if (i > 0) System.out.print(", ");
            System.out.print(newsTitles.elementAt(i));
        }
        System.out.print("\nPeople here: ");
	...
	"create <thing>: create a new news(only for admin).  Prompts for description \n" +
	"destroy <thing>: destroy a news(only for admin).\n" + 
	"open <direction>: create an adjoining place(only for admin). Prompts for input\n"+
	"close <direction>: close an exit from this place(only for admin).\n" +
	"link <direction>: create an exit to an existing place,\n" +
	"     perhaps on another server.  Will prompt for input.\n" +
	"dump <filename>: save server state(only for admin).  Prompts for password\n" +
	"quit: leave the Mud\n" +
	"delete: remove old news(only for admin)\n" +
	"find <dd-mm-yyyy>: find news by date\n" +
